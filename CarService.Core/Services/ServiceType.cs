...
public class ServiceType
{
    ...
    public static Result<ServiceType> Create(Guid id, string name)
    {
        if (id == Guid.Empty)
            return Result.Failure<ServiceType>("Id can't be empty");

        if (string.IsNullOrWhiteSpace(name))
            return Result.Failure<ServiceType>("Name can't be empty");
